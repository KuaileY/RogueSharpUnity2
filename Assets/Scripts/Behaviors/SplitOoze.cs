...
public class SplitOoze : IBehavior
{
    ...
    private RogueSharp.Cell FindClosestUnoccupiedCell( DungeonMap dungeonMap, int x, int y )
    {
        for ( int i = 1; i < 5; i++ )
        {
        foreach (RogueSharp.Cell cell in dungeonMap.GetBorderCellsInArea( x, y, i ) )
        {
            if ( cell.IsWalkable )
            {
                return cell;
